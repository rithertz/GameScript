...
void IntegerLiteralExpr::accept(ASTVisitor& visitor)
{
    visitor.visit(*this);
}
...
void BoolLiteralExpr::accept(ASTVisitor& visitor)
{
    visitor.visit(*this);
}
...
std::string UnaryExpr::toString() const
{
    return "(" +
           unaryOpToString(op_) +
           (operand_ ? operand_->toString() : "null") +
           ")";
}

void SensoryConditionExpr::accept(ASTVisitor& visitor)
{
    visitor.visit(*this);
}

std::string SensoryConditionExpr::toString() const
{
    return sensoryTypeToString(sensoryType_);
}
...
{
    return "player move " +
           moveDirToString(direction_) +
           " " +
           (distance_ ? distance_->toString() : "0");
}

void TurnStmt::accept(ASTVisitor& visitor)
{
    visitor.visit(*this);
}
...
{
    return "player turn " +
           turnDirToString(direction_) +
           " " +
           (degrees_ ? degrees_->toString() : "0");
}

void ActionStmt::accept(ASTVisitor& visitor)
{
    visitor.visit(*this);
}
...
{
    return "repeat " +
           (countExpr_ ? countExpr_->toString() : "0");
}
...
void FunctionCallStmt::accept(ASTVisitor& visitor)
{
    visitor.visit(*this);
}
...
void WhenStmt::accept(ASTVisitor& visitor)
{
    visitor.visit(*this);
}
...
{
    return "Program(" +
           std::to_string(statements_.size()) +
           " stmts)";
}
...
void ASTPrinter::visit(MoveStmt& node)
{
    oss_ << getIndentString()
         << "|-- Move: "
         << moveDirToString(node.getDirection())
         << "\n";
...
void ASTPrinter::visit(TurnStmt& node)
{
    oss_ << getIndentString()
         << "|-- Turn: "
         << turnDirToString(node.getDirection())
         << "\n";
...
void ASTPrinter::visit(ActionStmt& node)
{
    oss_ << getIndentString()
         << "|-- Action: "
         << actionKindToString(node.getAction())
         << "\n";
}
